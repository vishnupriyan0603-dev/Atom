...
/**
 * Builds a fully-wired AtomBrain (memory, knowledge, tools, and the
 * multi-provider ModelManager — Groq/Gemini/OpenAI/Anthropic/local Ollama)
 * from the shared root `.env`.
 *
 * Single source of truth for provider/model wiring so every caller that
 * needs to run a chat completion — the backend REST API (Web + Mobile) and
 * the standalone frontend chat page — resolves providers identically,
 * per the AGENTS.md cross-client parity rule. Extracted from what was a
 * CodeIgniter-only private method on App\Services\AiChatService.
 */
class AtomBrainFactory
{
    public static function create(string $workspaceRoot): AtomBrain
    {
