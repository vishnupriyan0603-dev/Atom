...
class ModelManager
{
    ...
    public function generateForProvider(string $provider, array $messages): ModelResponse
    {
        ...
        foreach ($attempts as $index => $model) {
            // Skip unavailable fallback models (e.g. a stopped local Ollama),
            // but always attempt the primary/requested model first.
        ...
        return new ModelResponse(false, '', $lastError ?: 'All models failed');
    }
