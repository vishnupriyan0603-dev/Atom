...
class Knowledge extends BaseApiController
{
    ...
    public function documents()
    {
        ...
        try {
            ...
            log_message('error', '[ATOM DOCUMENTS] Failed to fetch documents: ' . $e->getMessage());
            return $this->respondError('Failed to fetch documents', 500);
        }
    }
    ...
    public function upload()
    {
        ...
        try {
            ...
            if (!$result['success']) {
                ...
                return $this->respondError('Import process failed', 500);
            }
            ...
            return $this->respondError('System Exception during import', 500);
        }
    }
    ...
    public function deleteDocument($id = null)
    {
        ...
        try {
            ...
            log_message('error', '[ATOM DELETE] Failed to delete document: ' . $e->getMessage());
    ...
    public function trainDocument($id = null)
    {
        ...
        $path = $doc['path'];
        if (!file_exists($path)) {
            log_message('error', '[ATOM TRAIN] Source file missing on disk for doc ' . $id);
            return $this->respondError('Source file missing on disk', 500);
        }
        ...
        try {
            ...
            log_message('error', '[ATOM TRAIN] Text extraction failed for doc ' . $id . ': ' . $e->getMessage());
            return $this->respondError('Text extraction failed', 500);
        }
        ...
        try {
            ...
            log_message('error', '[ATOM TRAIN] AI understanding failed for doc ' . $id . ': ' . $e->getMessage());
            return $this->respondError('AI understanding failed', 500);
        }
        ...
        // 4. Make the trained understanding retrievable: replace any previous
        //    summary chunk with the new one so chat retrieval can use it.
        $delChunk = $pdo->prepare("DELETE FROM atom_document_chunks WHERE document_id = ? AND page_number = 0");
        $delChunk->execute([(int)$id]);

        $insChunk = $pdo->prepare(
            "INSERT INTO atom_document_chunks (document_id, page_number, section_title, chunk_text)
             VALUES (?, 0, 'ATOM Understanding Summary', ?)"
        );
        $insChunk->execute([(int)$id, $understanding]);
