...
<body class="text-[#f0f4f8] h-screen flex">
  ...
  <script>
      ...
      try {
        ...
        if (json.success && json.data && json.data.length > 0) {
          list.innerHTML = json.data.map(c => `
            <button onclick="selectChat(${c.id}, '${c.title}')" class="w-full text-left px-3 py-2 rounded-xl text-xs font-semibold text-gray-400 hover:bg-[#1e2735] hover:text-white truncate block ${c.id === activeChatId ? 'bg-[#1e2735] text-white' : ''}">
    ...
    async function selectChat(id, title) {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/chats/' + id + '/messages');
        const json = await resp.json();
          ...
          messagesBox.innerHTML = json.data.map(m => {
            ...
            return `
              <div class="flex flex-col space-y-2 max-w-3xl ${isUser ? 'ml-auto' : 'mr-auto'}">
                ...
                <div class="p-4 rounded-2xl border ${border} ${bg} text-xs text-gray-300 leading-relaxed font-mono whitespace-pre-wrap">${m.content}</div>
              </div>
            `;
          }).join('');
    ...
    async function sendMessage(event) {
      ...
      messagesBox.innerHTML += `
        <div class="flex flex-col space-y-2 max-w-3xl ml-auto">
          ...
          <div class="p-4 rounded-2xl border border-blue-500/10 bg-[#1a2332] text-xs text-gray-300 leading-relaxed font-mono whitespace-pre-wrap">${text}</div>
        </div>
      `;
      messagesBox.scrollTop = messagesBox.scrollHeight;

      try {
        const resp = await fetch('http://localhost:8080/api/chats/' + activeChatId + '/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
