...
<body class="text-[#f0f4f8] h-screen flex overflow-hidden">
  ...
  <script>
    async function loadDocuments() {
      try {
        const resp = await fetch('http://localhost:8080/api/knowledge/documents');
        ...
        if (json.success && json.data && json.data.length > 0) {
          tbody.innerHTML = json.data.map(doc => `
            <tr class="hover:bg-[#16202e]/30 transition-all">
              <td class="p-4 font-bold text-white">${doc.title || doc.filename}${doc.trained_at ? ' <span class="ml-1 px-1.5 py-0.5 rounded text-[9px] font-black bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 uppercase">Trained</span>' : ''}</td>
              <td class="p-4 text-gray-400 font-mono">${doc.filename}</td>
    ...
    async function trainDocument(id) {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/knowledge/documents/' + id + '/train', {
          method: 'POST'
        });
        const json = await resp.json();
        if (json.success) {
    ...
    function showUnderstandingModal(summary) {
      ...
      overlay.innerHTML = `
        <div style="width:640px;max-width:92%;max-height:80vh;overflow-y:auto;background:#11151c;border:1px solid #1e2838;border-radius:16px;box-shadow:0 20px 50px rgba(0,0,0,0.6);">
          ...
          <div style="padding:24px;color:#cbd5e1;font-size:12.5px;line-height:1.7;white-space:pre-wrap;">${summary.replace(/</g, '&lt;')}</div>
        </div>`;
    ...
    async function deleteDocument(id) {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/knowledge/documents/' + id, {
          method: 'DELETE'
        });
        const json = await resp.json();
        if (json.success) {
    ...
    async function uploadDocument() {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/knowledge/upload', {
          method: 'POST',
          body: formData
        });
        const json = await resp.json();
