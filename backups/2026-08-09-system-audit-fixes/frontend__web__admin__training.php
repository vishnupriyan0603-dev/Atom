...
<body class="text-[#f0f4f8] h-screen flex overflow-hidden">
  ...
  <script>
    async function loadTrainingData() {
      try {
        const resp = await fetch('http://localhost:8080/api/settings/training_records');
          ...
          tbody.innerHTML = json.data.map(item => `
            <tr class="hover:bg-[#16202e]/30 transition-all">
              <td class="p-4 space-y-1">
                <div class="font-bold text-white leading-tight">Q: ${item.user_input}</div>
                <p class="text-[11px] text-gray-500 font-mono line-clamp-2">A: ${item.preferred_response}</p>
              </td>
              <td class="p-4"><span class="px-2 py-0.5 rounded font-bold bg-emerald-500/10 text-emerald-400 text-[10px] uppercase">${item.category || 'General'}</span></td>
              <td class="p-4">
                <span class="px-2 py-0.5 rounded font-bold text-[10px] uppercase ${item.quality === 'VERIFIED' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'}">${item.quality}</span>
              </td>
    ...
    async function loadDuplicates() {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/settings/duplicates');
        const json = await resp.json();
            ...
            <div class="p-4 rounded-xl bg-[#080a0d] border border-[#1e2838] space-y-3">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-gray-400">
                <div class="space-y-1">
                  ...
                  <p class="leading-relaxed">${grp.question_a}</p>
                </div>
                <div class="space-y-1">
                  <span class="font-bold text-white block">Question B:</span>
                  <p class="leading-relaxed">${grp.question_b}</p>
                </div>
              </div>
    ...
    async function rejectRecord(id) {
      ...
      try {
        const resp = await fetch('http://localhost:8080/api/settings/training_records/' + id, { method: 'DELETE' });
        if(resp.ok) {
